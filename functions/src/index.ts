...
async function sendNotification(
  ...
  if (!userDoc.exists) {
    const logMessage = `[NotificationHelper] ${userType} ${userId} no encontrado.`;
...
const interpretarBusquedaOutputSchema = z.object({
  ...
  idiomaDetectado: z.string().describe(
    "El código ISO 639-1 del idioma detectado (ej: es, en)."
  ),
});
...
export const createImmediateServiceRequest = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
    ...
    const {providerId, selectedServices, totalAmount, location, metodoPago,
      codigoPromocion} = data;

    if (!providerId || !Array.isArray(selectedServices) ||
      selectedServices.length === 0 || typeof totalAmount !== "number" ||
      !location || !metodoPago) {
    ...
    if (codigoPromocion && typeof codigoPromocion === "string") {
      const promoQuery = db.collection("promociones")
        .where("codigoPromocional", "==", codigoPromocion).limit(1);
      ...
      const promoExpired = promoData.fechaExpiracion &&
        promoData.fechaExpiracion.toMillis() < now.toMillis();
      const noUsesLeft = typeof promoData.usosDisponibles === "number" &&
      ...
      const desc = `Usuario aplicó promoción "${codigoPromocion}". ` +
        `Descuento: $${montoDescuento.toFixed(2)}`;
      await logActivity(usuarioId, "usuario", "PROMO_APLICADA", desc,
        {tipo: "solicitud_servicio", id: nuevaSolicitudRef.id}, promoAplicada);
    }
    ...
    const userBlocksProviderQuery = db.collection("bloqueos")
      .where("bloqueadorRef", "==", usuarioId)
      .where("bloqueadoRef", "==", providerId).limit(1);
    ...
    const providerBlocksUserQuery = db.collection("bloqueos")
      .where("bloqueadorRef", "==", providerId)
      .where("bloqueadoRef", "==", usuarioId).limit(1);
    ...
    const nuevaSolicitudData: Omit<ServiceRequest, "id" | "serviceType"> &
      { serviceType: "fixed" } = {
        usuarioId: usuarioId,
        ...
        `${selectedServices.map((s: {title: string}) => s.title).join(", ")}`,
        serviceType: "fixed",
    ...
    const logDesc = `Usuario ${usuarioId} creó y pagó una solicitud ` +
      `#${nuevaSolicitudRef.id} para ${providerId}. ` +
      `Total: $${montoFinal.toFixed(2)}.`;
    await logActivity(
      usuarioId, "usuario", "SOLICITUD_CREADA", logDesc,
      {tipo: "solicitud_servicio", id: nuevaSolicitudRef.id},
      {totalAmount: montoFinal, metodoPago, promoAplicada}
    );

    await sendNotification(
      providerId, "prestador", "¡Nuevo Servicio Inmediato!",
      `Has recibido un nuevo servicio inmediato de ${usuarioId}. ¡Prepárate!`,
      {solicitudId: nuevaSolicitudRef.id}
    );

    const successMsg = `Servicio solicitado exitosamente. ` +
      `Total: $${montoFinal.toFixed(2)}.`;
    return {
      success: true,
      message: successMsg,
...
export const onServiceStatusChangeSendNotification = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    if (newValue.status === previousValue.status &&
      newValue.paymentStatus === previousValue.paymentStatus) return null;

    const logMsg = `[FCM] Solicitud ${solicitudId}. Antes: ` +
      `${previousValue.status}. Después: ${newValue.status}.`;
    functions.logger.log(logMsg);
    ...
    if (newValue.status !== previousValue.status) {
      ...
      case "agendado":
        if (newValue.isRecurringAttempt &&
          ...
          cuerpoNotif = `El usuario ${usuarioId} quiere reactivar el ` +
            `servicio "${serviceTitle}".`;
        } else if (!newValue.isRecurringAttempt &&
          previousValue.status !== "pendiente_confirmacion_usuario") {
          targetUserId = prestadorId; targetUserType = "prestador";
          tituloNotif = "Nueva Solicitud de Servicio";
          cuerpoNotif = `Has recibido una nueva solicitud para ` +
            `"${serviceTitle}".`;
        } else {
        ...
        if (newValue.serviceDate && newValue.serviceTime) {
          try {
            const [year, month, day] =
              newValue.serviceDate.split("-").map(Number);
            const [hour, minute] =
              newValue.serviceTime.split(":").map(Number);
            const serviceDateTime = new Date(year, month - 1, day, hour, minute);
            const reminderTime = new Date(serviceDateTime.getTime() -
              HORAS_ANTES_RECORDATORIO_SERVICIO * 60 * 60 * 1000);

            if (reminderTime.getTime() > Date.now()) {
              const prestadorDoc =
                await db.collection("prestadores").doc(prestadorId).get();
              ...
              const logDesc =
                `Recordatorio programado para servicio ${solicitudId}.`;
              await logActivity("sistema", "sistema",
                "NOTIFICACION_RECORDATORIO_PROGRAMADA", logDesc,
      ...
      case "rechazada_prestador": case "cancelada_prestador": {
        const statusText = newValue.status === "rechazada_prestador" ?
          "rechazada" : "cancelada";
        targetUserId = usuarioId; targetUserType = "usuario";
        tituloNotif =
          `Cita ${statusText.charAt(0).toUpperCase() + statusText.slice(1)}`;
        cuerpoNotif = `Tu cita para "${serviceTitle}" ha sido ` +
    ...
    if (newValue.paymentStatus !== previousValue.paymentStatus &&
    ...
    if (sendStdNotification && targetUserId && targetUserType &&
      tituloNotif && cuerpoNotif) {
      await sendNotification(targetUserId, targetUserType, tituloNotif,
        cuerpoNotif, {
          solicitudId,
...
export const logSolicitudServicioChanges = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    const now = admin.firestore.Timestamp.now();
    const updatesToServiceRequest:
      Partial<ServiceRequest> & {updatedAt?: admin.firestore.Timestamp} =
      {updatedAt: now};


    if (beforeData.status !== afterData.status) {
      let logAction: ActivityLogAction = "CAMBIO_ESTADO_SOLICITUD";
      let logDescription = `Solicitud ${solicitudId} cambió de ` +
        `${beforeData.status} a ${afterData.status}.`;

      const newStatus = afterData.status as ServiceRequestStatus;
      const isFinalState = ESTADOS_FINALES_SERVICIO
        .includes(newStatus as EstadoFinalServicio);
      const isCancelledState = newStatus.startsWith("cancelada_") ||
      ...
      if (isFinalState && !isCancelledState) {
        ...
        logDescription = `Servicio ${solicitudId} finalizado por ` +
          `${actorRol}. Estado final: ${newStatus}.`;
      } else if (isCancelledState) {
        logAction = "SERVICIO_CANCELADO";
        logDescription = `Servicio ${solicitudId} cancelado por ` +
          `${actorRol}. Estado: ${newStatus}.`;
      } else if (isDisputeState) {
        logAction = "SERVICIO_EN_DISPUTA";
        logDescription = `Servicio ${solicitudId} puesto en disputa por ` +
          `${actorRol}.`;
      }

      await logActivity(
        actorId, actorRol, logAction, logDescription,
      ...
      if (isFinalState && !ESTADOS_FINALES_SERVICIO
        .includes(beforeData.status as EstadoFinalServicio)) {
    ...
    const isFinalizedState = ESTADOS_FINALES_SERVICIO
      .includes(afterData.status as EstadoFinalServicio);
    const wasNotFinalizedBefore = !ESTADOS_FINALES_SERVICIO
      .includes(beforeData.status as EstadoFinalServicio);
    ...
    if (isFinalizedState && wasNotFinalizedBefore) {
      ...
      const relationshipRef =
        db.collection("relacionesUsuarioPrestador").doc(relationshipId);
      const providerDoc =
        await db.collection("prestadores").doc(afterData.prestadorId).get();
    ...
    const shouldReleasePayment = (isFinalizedState && wasNotFinalizedBefore &&
      afterData.paymentStatus === "retenido_para_liberacion") ||
      (beforeData.paymentStatus === "retenido_para_liberacion" &&
      afterData.paymentStatus === "liberado_al_proveedor" && isFinalizedState &&
    ...
    if (shouldReleasePayment) {
      ...
      const detallesFinancierosNuevos: DetallesFinancieros =
        {...(afterData.detallesFinancieros as DetallesFinancieros || {})};
      ...
      if (montoTotalPagadoPorUsuario > 0 &&
        ...
        const montoNeto = montoTotalPagadoPorUsuario -
        ...
        const comisionApp = montoTotalPagadoPorUsuario *
          COMISION_APP_SERVICIOMAP_PORCENTAJE;
        detallesFinancierosNuevos.comisionAppMonto = comisionApp;
        detallesFinancierosNuevos.aporteFondoFidelidadMonto = comisionApp *
        ...
        await logActivity("sistema", "sistema", "PAGO_LIBERADO", logDesc,
          {tipo: "solicitud_servicio", id: solicitudId},
          detallesFinancierosNuevos);

        const pointsEarned =
          Math.floor(montoTotalPagadoPorUsuario / FACTOR_CONVERSION_PUNTOS);
        if (pointsEarned > 0) {
          ...
          await logActivity(
            afterData.usuarioId, "usuario", "PUNTOS_FIDELIDAD_GANADOS",
            logDescPts,
            {tipo: "usuario", id: afterData.usuarioId},
            {puntos: pointsEarned, servicioId}
          );
        }
        ...
        if (providerDoc.exists) {
          ...
          if (providerData.referidoPor) {
            ...
            const comisionEmbajador = comisionAppMonto *
...
export const onQuotationResponseNotifyUser = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    const beforeData = change.before.data() as SolicitudCotizacionData |
      undefined;
    const afterData = change.after.data() as SolicitudCotizacionData |
      undefined;
    ...
    if (afterData.estado === "precio_propuesto_al_usuario" &&
      beforeData.estado === "pendiente_revision_prestador") {
      notifTitle = "¡Cotización Actualizada!";
      notifBody = `${nombrePrestador} ha propuesto un precio de ` +
        `$${afterData.precioSugerido} para ${tituloServicio}.`;
      logAction = "COTIZACION_PRECIO_PROPUESTO";
      logDesc = `${nombrePrestador} propuso precio para ` +
        `cotización ${cotizacionId}.`;
    } else if (afterData.estado === "rechazada_prestador" &&
      beforeData.estado === "pendiente_revision_prestador") {
      notifTitle = "Cotización Rechazada";
      notifBody = `${nombrePrestador} ha rechazado tu solicitud para ` +
        `${tituloServicio}.`;
    ...
    if (notifTitle && notifBody) {
      await sendNotification(usuarioId, "usuario", notifTitle,
        notifBody, {cotizacionId: cotizacionId});
    }
    if (logAction && logDesc) {
      await logActivity(prestadorId, "prestador", logAction, logDesc,
        {tipo: "solicitud_cotizacion", id: cotizacionId});
    }
    return null;
  });

export const acceptQuotationAndCreateServiceRequest = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError("unauthenticated",
        "Autenticación requerida.");
    ...
    const cotizacionRef =
      db.collection("solicitudes_cotizacion").doc(cotizacionId);
    try {
      return await db.runTransaction(async (transaction) => {
        ...
        if (typeof cotizacionData.precioSugerido !== "number" ||
        ...
        const userBlocksProviderQuery = db.collection("bloqueos")
          .where("bloqueadorRef", "==", usuarioId)
          .where("bloqueadoRef", "==", prestadorId).limit(1);
        const userBlocksProviderSnap =
          await transaction.get(userBlocksProviderQuery);
        if (!userBlocksProviderSnap.empty) {
          const msg = "No puedes contratar a este proveedor porque " +
            "lo has bloqueado.";
          throw new functions.https.HttpsError("permission-denied", msg);
        }
        const providerBlocksUserQuery = db.collection("bloqueos")
          ...
          .where("bloqueadoRef", "==", usuarioId).limit(1);
        const providerBlocksUserSnap =
          await transaction.get(providerBlocksUserQuery);
        if (!providerBlocksUserSnap.empty) {
          throw new functions.https.HttpsError("permission-denied",
        ...
        const serviceDateStr = `${tomorrow.getFullYear()}-`+
        ...
        transaction.update(cotizacionRef,
          {estado: "convertida_a_servicio", fechaRespuestaUsuario: ahora});

        const logDesc = `Usuario aceptó cotización ${cotizacionId}. ` +
          `Nueva solicitud ID: ${nuevaSolicitudRef.id}.`;
        await logActivity(usuarioId, "usuario", "COTIZACION_ACEPTADA_USUARIO",
          logDesc, {tipo: "solicitud_cotizacion", id: cotizacionId});
        const notifBody = "El usuario ha aceptado tu cotización y se ha " +
          "generado una nueva solicitud de servicio.";
        await sendNotification(prestadorId, "prestador",
          "¡Cotización Aceptada!", notifBody,
      ...
      functions.logger.error(`Error al aceptar cotización ${cotizacionId}:`,
        httpsError);
      if (httpsError.code) throw httpsError;
      throw new functions.https.HttpsError("internal",
        "Error al procesar.", httpsError.message);
    }
  });
